...
public class DashboardController implements Initializable {
    ...
    private void handleHistory() {
        SceneManager.switchScene("history.fxml", "Fishermen Weather App - Trip History");
    }
    ...
    private void displaySafetyAssessment(TripCalculation calculation) {
        ...
        if (!calculation.getWarnings().isEmpty()) {
            ...
            for (String warning : calculation.getWarnings()) {
                HBox warningItem = new HBox(5);
                ImageView warningIcon = new ImageView(new Image(getClass().getResourceAsStream("/images/warning-small-icon.png")));
                warningIcon.setFitHeight(12);
                warningIcon.setFitWidth(12);
                Text warningText = new Text(warning);
                warningText.getStyleClass().add("warning-text");
                warningItem.getChildren().addAll(warningIcon, warningText);
                warningsList.getChildren().add(warningItem);
            }
